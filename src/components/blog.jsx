...
const BlogPage = () => {
  ...
  const handleLike = async() => {
    try {
      const response = await axios.post('http://localhost:3002/api/v1/like/add', {
        blog_id: id,
      }, { withCredentials: true });
  ...
  const handleAddComment = async () => {
    if (newComment.trim() !== '') {
      try {
        const response = await axios.post('http://localhost:3002/api/v1/comment/add_comment', {
          blog_id: id,
  ...
  const handleReply = async(commentId, replyText) => {
   ...
   console.log("Id -> ",id);
   try {
    const response = await axios.post('http://localhost:3002/api/v1/comment/add_reply', {
  ...
  const handleStarBlog = async () => {
    try {
      const response = await axios.post('http://localhost:3002/api/v1/blog/star_blog', {
        blog_id: id,
      }, { withCredentials: true });
